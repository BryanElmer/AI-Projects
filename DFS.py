...
class State:
    ...
    def __init__(self, posX, posY, goalX, goalY, moves, cost):
        self.posX = posX
        self.posY = posY
        self.goalX = goalX
        self.goalY = goalY
...
def search(board, state):
    frontier = LifoQueue(maxsize = 0) # ["a", "1"]
    reached = {}
    nodesExplored = 0
    ...
    while frontier.not_empty:
        ...
        for action in actions:
            ...
            newCost = currentNode.cost + board.config[action[0]][action[1]]
            ...
            if actionNode.isGoal():
                return actionNode.moves, nodesExplored
            if not(alphaPos in reached.keys()) or newCost < reached[alphaPos].cost: # check if action is not in reached, or cheaper path, then add to frontier
                # Add into reached set, with corresponding path cost (current cost + new action cost)
                reached[alphaPos] = actionNode
                frontier.put(actionNode)
        if frontier.empty():
            return node.moves, nodesExplored
...
def run_DFS():
    ...
    moves, nodesExplored = search(board, state) #For reference
